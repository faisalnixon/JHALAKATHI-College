...
export async function getLatestNotices(
  ...
  /*
   * `posts` is already sorted newest-first in buildNoticePosts, and this
   * intentionally ignores hashtags — it's for a general "latest
   * notices" feed (the homepage) that mixes every category together.
   * Each post appears once here regardless of how many hashtags it
   * carries, since `posts` already holds one entry per post.
   */
